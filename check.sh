...
# tmp/ 403, index 200, app.js 200; with credentials: 200 for API and audio after
# login, 403 for another user's prepared download.
...
curl -s -o /dev/null -w "tmp/:         %{http_code}  (expect 403 or 404)\n" "$BASE/tmp/u-nobody/x.zip"
...
if [ -n "${2:-}" ]; then
  ...
  curl -s -b "$C" -o /dev/null -w "other's zip:  %{http_code}  (expect 403 or 404 – never 200)\n" "$BASE/tmp/u-nobody/x.zip"
  curl -s -b "$C" -o /dev/null -w "download API: %{http_code}  (expect 200)\n" "$BASE/getData.php?action=downloadStatus"
  rm -f "$C"
fi
